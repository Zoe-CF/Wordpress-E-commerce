...
if( ! class_exists( 'Remax_Store_Repeater_Control' ) ) {
    /**
     * Repeater control
     */
    class Remax_Store_Repeater_Control extends WP_Customize_Control {
    	...
    	public function __construct( $manager, $id, $args = array() ) {
    		...
    		$this->row_label = array(
    			'type' => 'text',
    			'value' => esc_attr__( 'row', 'remax-store' ),
    			'field' => false,
    		);
    		...
    		if ( empty( $this->button_label ) ) {
    			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'remax-store' ), $this->row_label['value'] );
    		}
    				...
    				if ( isset( $field['type'] ) ){
    					if( 'color' === $field['type'] ){ 
                           //wp_enqueue_script( 'wp-color-picker' );
                            wp_enqueue_script( 'wp-color-picker-alpha', REMAX_STORE_THEME_URI. 'themerelic/customizer/custom-controls/repeater/cs-wp-color-picker.min.js', array('wp-color-picker'), '4.7',true );
                            
                            wp_enqueue_style( 'wp-color-picker' );
    					}elseif( 'font' === $field['type'] ){
                            wp_enqueue_style( 'font-awesome', REMAX_STORE_THEME_URI. 'assets/library/font-awesome/css/font-awesome.css', array(), '4.7' );
    					}
    				}
    			}
    		}
            wp_enqueue_script( 'remax-store-repeater', REMAX_STORE_THEME_URI . 'themerelic/customizer/custom-controls/repeater/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
    		wp_enqueue_style( 'remax-store-repeater', REMAX_STORE_THEME_URI . 'themerelic/customizer/custom-controls/repeater/repeater.css', null );
    	}
    	...
    	protected function render_content() {
    		...
    		<?php if ( isset( $this->choices['limit'] ) ) : ?>
    			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'remax-store' ), esc_html( $this->choices['limit'] ) ); ?></p>
    		<?php endif; ?>
    		...
    		<script type="text/html" class="customize-control-repeater-content">
    			...
    			<li class="repeater-row minimized" data-row="{{{ index }}}">
    				...
    				<div class="repeater-row-content">
    					<# _.each( data, function( field, i ) { #>
    						...
    						<div class="repeater-field repeater-field-{{{ field.type }}}">
								...
								<# } else if ( 'mulselect' === field.type ) { #>
									...
									<label>
										...
										<# if ( field.description ) { #>
											<span class="description customize-control-description">{{ field.description }}</span>
										<# } #>
										<select data-field="{{{ field.id }}}" multiple>
											<# _.each( field.choices, function( choice, i ) {  #>
												<option value="{{{ i }}}" default= {{{field.default }}} <# if ( _.contains(field.default,i) ) { #> selected="selected" <# } #>>{{ choice }}</option>
											<# }); #>
										</select>
									</label>
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker" data-alpha="true" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'remax-store' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
    							...
    							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
    								...
    								<figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'remax-store' ); ?>" >
    									<# if ( field.default ) { #>
    										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
    										<img src="{{{ defaultImageURL }}}">
    									<# } else { #>
    										<?php esc_html_e( 'No Image Selected', 'remax-store' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'remax-store' ); ?></button>
    									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'remax-store' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'remax-store' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_html_e( 'Change Image', 'remax-store' ); ?>
    										<# } else { #>
    											<?php esc_html_e( 'Add Image', 'remax-store' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    							...
    							<# } else if ( field.type === 'upload' ) { #>
    								...
    								<figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'remax-store' ); ?>" >
    									<# if ( field.default ) { #>
    									...
    									<# } else { #>
    										<?php esc_html_e( 'No File Selected', 'remax-store' ); ?>
    									<# } #>
    								</figure>
    
    								<div class="actions">
    									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
    									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'remax-store' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'remax-store' ); ?>" >
    										<# if ( field.default ) { #>
    											<?php esc_html_e( 'Change File', 'remax-store' ); ?>
    										<# } else { #>
    											<?php esc_html_e( 'Add File', 'remax-store' ); ?>
    										<# } #>
    									</button>
    									<# if ( field.default.id ) { #>
    										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
    					...
    					<# }); #>
    					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'remax-store' ); ?></button>
    				</div>
    			</li>
    		</script>
    		<?php
        ...
        function get_icon_list(){
    	    $icons = load_template( wp_normalize_path( get_template_directory() . '/themerelic/customizer/fontawesome.php' )); 
            if( $icons ){ ?>
